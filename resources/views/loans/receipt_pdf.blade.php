...
<head>
  ...
  <style>
    ...
    .attachment-group { border: 1px solid #dbe4f3; background: #f8fbff; padding: 8px; margin-bottom: 8px; }
    .attachment-group h4 { margin: 0 0 6px; font-size: 10px; text-transform: uppercase; letter-spacing: 0.06em; color: #1e3a8a; }
    ...
    .meta-wrap, .items, .sign-row, .attachments, .attachment-group, .attachment-card { page-break-inside: avoid; }
    .attachments h3, .attachment-group h4 { page-break-after: avoid; }
  </style>
</head>
<body>
  <div class="header">
  ...
  <div class="summary">
    <span class="chip">Kode Pinjam: {{ $batch }}</span>
    <span class="chip">Total Unit: {{ $items->sum('quantity') }}</span>
    <span class="chip">Jenis Barang: {{ $items->count() }}</span>
  </div>
    ...
    <tr>
      <td style="width:50%;">
        <table class="meta-box">
          ...
          <tr><td>Nama</td><td>{{ $borrower }}</td></tr>
          <tr><td>Unit Kerja</td><td>{{ $unit }}</td></tr>
          @if($contact)
            <tr><td>Kontak</td><td>{{ $contact }}</td></tr>
          @endif
          @if($activity_name)
            <tr><td>Kegiatan</td><td>{{ $activity_name }}</td></tr>
          @endif
        </table>
      </td>
      <td style="width:50%;">
        <table class="meta-box">
          ...
          <tr><td>Tgl Pinjam</td><td>{{ \Illuminate\Support\Carbon::parse($loan_date)->format('d M Y') }}</td></tr>
          <tr><td>Rencana Kembali</td><td>{{ $return_plan ? \Illuminate\Support\Carbon::parse($return_plan)->format('d M Y') : '-' }}</td></tr>
  ...
  <table class="items">
    ...
    <tfoot>
      <tr>
        ...
        <td class="align-center">{{ $items->sum('quantity') }}</td>
      </tr>
    </tfoot>
  </table>
  ...
  <div class="sign-row">
    <div class="sign">Peminjam
      <div class="sign-name">{{ $borrower }}</div>
    </div>
                ...
                @foreach($chunk as $index => $path)
                  ...
                  <div class="attachment-card">
                    ...
                    @if($absolute && file_exists($absolute))
                      <img src="{{ $absolute }}" alt="{{ $label }} {{ $photoNumber }}">
                    @else
